...
subsets <- c(1:5, 10, 15, 20, 25, 30, 35, 37)
...
confusionMatrix(reference = as.factor(testingData$ruff_turnout), data = as.factor(as.vector(pred)), mode='everything', positive='MM')
something = as.factor(testingData$ruff_turnout)
something2 = as.factor(as.vector(pred))
test.df <- as.data.frame(testingData)
pred.df <- as.data.frame(pred)
test_temp <- testingData$ruff_turnout

identical(levels(test.df$ruff_turnout), levels(pred.df))

confusionMatrix(reference = test.df$ruff_turnout, data = pred.df, mode='everything', positive='MM')
```
...
models_compare <- resamples(list(RF=model_rf, XGBDART=model_xgbDART, MARS=model_mars, SVM=model_svmRadial))
...
plot(bestFitSummary$cp, type = "l")
plot(bestFitSummary$bic, type = "l")
plot(bestFitSummary$rss, type = "l")
plot(bestFitSummary$rsq, type = "l")
bestFitSummary
```
...
```{r}
pr.out = prcomp(trainingData, scale = TRUE)
summary(pr.out)

sd.data=scale(trainingData)
