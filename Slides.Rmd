...
acs14 = lapply(acs14, function (x) gsub("%", "", x)) %>%
  ...
  as.tibble() %>%
  mutate(votingPop = `Total population`*(EighteenTo24 + TwentyfiveTo44 + FortyFiveTo54 + FiftyfiveTo64 + SixtyfiveTo74 + Seventyfive_plus), ruff_turnout = tot_votes/votingPop)
```
...
```{r}
#model_QRNN = train(ruff_turnout ~ ., data=trainingDataMinusDR, method='qrnn')
#plot(model_QRNN)
```
